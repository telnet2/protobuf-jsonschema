...
function Compiler(filenames) {
    this.messages = {};
    this.enums = {};
    this.files = filenames;
    this.schema = this.open(filenames[0]);
}

Compiler.prototype.resolveFile = function (filename) {
    let foundFiles = this.files.filter(f => {
...
Compiler.prototype.open = function (filename) {
    ...
    if (fs.existsSync(filename)) {
        ...
        schema.imports.forEach(function (i) {
            this.open(this.resolveFile(i) || path.resolve(path.dirname(filename), i));
        }, this);
